    ...
    $app->group('/organization_structure', function () use ($app) {
        $app->group('/api', function () use ($app) {
            ...
            $app->group('/department', function () use ($app) {
                ...
                $app->get('', \librarycontroller::class . ':get_department');
                $app->post('', \librarycontroller::class . ':post_department');
                $app->patch('', \librarycontroller::class . ':patch_department');
                $app->delete('', \librarycontroller::class . ':delete_department');
            ...
            $app->group('/staff', function () use ($app) {
                ...
                $app->group('/concatenation', function () use ($app) {
                    $app->get('', \librarycontroller::class . ':get_staff');
                    $app->post('', \librarycontroller::class . ':post_staff');
                    $app->patch('', \librarycontroller::class . ':patch_staff');
                    $app->delete('', \librarycontroller::class . ':delete_staff');
                    $app->group('_export', function () use ($app) {
                        $app->get('/excel',  \librarycontroller::class . ':get_staff_export_excel');
                    });
                    $app->group('_import', function () use ($app) {
                        $app->get('/manual',  \librarycontroller::class . ':get_staff_import_manual');
                        $app->post('/read_sample',  \librarycontroller::class . ':get_staff_import_data');
                    });
                });
...
$app->group('/library', function () use ($app) {
    $app->group('/organization_structure', function () use ($app) {
        $app->group('/api', function () use ($app) {
            $app->group('/staff', function () use ($app) {
                $app->group('/relation', function () use ($app) {
                    $app->get('', \librarycontroller::class . ':get_staff_for_koha_relation');
                    $app->post('', \librarycontroller::class . ':post_staff_for_koha_relation');
                });
            });
        });
    });
